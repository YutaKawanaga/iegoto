...
| `shared/` | `packages/domain/src/shared/`（Id等の共有カーネル）+ 各パッケージ内のutil | 共有カーネル |
...
│                                  #   + apps/webビルド成果物の静的配信（単一オリジン。O-1配信構成）
...
## 8. 認証・セッション（セルフレビューで具体化。事故時の影響が最大の箇所）

Auth.js（Next.js前提）を使わない決定（T-4）に伴い、バックエンドでGoogle OAuthを実装する。
**フルスクラッチにはせず、OAuthプロトコル部分は検証済みの軽量ライブラリ（Arctic等のOAuth 2.0クライアント）を使う。**
自前で書くのはセッション管理とMember紐づけのアプリ固有部分のみ。

- フロー: `/auth/google` → Google（**state + PKCE必須**）→ `/auth/google/callback` → IDトークン検証（`sub`で
  `user_account`をUPSERT）→ セッション発行 → SPAへリダイレクト
- セッション: **DBセッション方式**（`session`テーブル: id・user_account_id・expires_at）。クッキーには
  ランダムなセッションIDのみを持たせる
  - 暗号化クッキー完結（plainer方式）ではなくDB方式にする理由: 即時失効（ログアウト・端末紛失時の全セッション破棄）が
    できることを優先。家族データ全開放の入口なので失効可能性 > DBアクセス1回の節約
- クッキー属性: `HttpOnly` / `Secure` / `SameSite=Lax` / `Path=/`。有効期限は30日のスライディング更新
- セキュリティチェックリスト（実装PRのレビュー観点）:
  - [ ] `state`検証（CSRF）と`PKCE`（コード横取り）を省略していない
  - [ ] IDトークンの`aud` / `iss` / 署名 / 有効期限を検証している（ライブラリ任せにしつつ設定値を確認）
  - [ ] ログイン成功時にセッションIDを再発行している（セッション固定攻撃対策）
  - [ ] コールバックのリダイレクト先を許可リストで固定している（open redirect禁止）
  - [ ] 変更系はすべてPOST（tRPC mutation）であり、SameSite=Laxと合わせてCSRFが成立しない
  - [ ] 招待トークン（S-2）・OAuth `state` をログに出力していない
- Google Calendar連携（T-7）のOAuthはログインとは**別の認可フロー**（scope: `calendar.readonly`、
  refresh token取得のため`access_type=offline`）。ログインのscopeは`openid email profile`のみに保ち、
  初回ログインで過剰な権限を求めない
