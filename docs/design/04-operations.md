...
> **2026-07-19更新**: 当面のホスティングは**Vercel無料構成**（`10-vercel-hosting.md`）を採用することにした。
> 本章のGCP構成は**破棄せず設計資産として保持**し、移行トリガー（10 §5: 商用化・PITRの必要・無料枠超過等）を
> 満たした時点でこの構成に移行する。「クラウドはprodのみ」「単一オリジン配信」の決定はVercel構成でも共通。
